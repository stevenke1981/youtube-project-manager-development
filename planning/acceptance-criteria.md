...
## AC-14 Portable NLE timeline

Given 專案有可用 video/audio assets，When 使用者新增、trim、移動、排序或移除 timeline clip，Then `timeline.json` atomic write 後可重載還原；素材檔案不被複製或刪除，非法 trim/overlap/asset reference 會顯示可行動錯誤。

## AC-15 FFprobe／FFmpeg pipeline

Given PATH 或設定中存在 ffprobe/ffmpeg，When 使用者探測素材或 export preview/final，Then UI 顯示 metadata、進度與 Saved output；取消可停止工作，輸出不覆寫既有檔案。工具缺失時 App 仍能編輯並顯示安裝指引。

## AC-16 YouTube OAuth upload

Given 本機已設定 OAuth client reference，When 使用者完成 metadata 並按下明確確認，Then upload job 使用 state+PKCE、顯示進度、可取消／重試，Token 不落入專案；未確認或 dry-run 不會傳送影片。
